...
import cern.c2mon.server.elasticsearch.IndicesTransport;
...
public class TagConfigDocumentIndexerTransport extends TagConfigDocumentIndexer {

  private static final String TYPE = "tag_config";

  private final ElasticsearchClientImpl client;
  private final IndicesTransport indicesTransport;

  private final String configIndex;

  @Autowired
  public TagConfigDocumentIndexerTransport(final ElasticsearchClientImpl client, final ElasticsearchProperties properties, final IndicesTransport indicesTransport) {
    this.client = client;
    this.indicesTransport = indicesTransport;
    this.configIndex = properties.getTagConfigIndex();
  }

  @Override
  void indexTagConfig(final TagConfigDocument tag) {
    if (!indicesTransport.exists(configIndex)) {
      indicesTransport.create(configIndex, TYPE, MappingFactory.createTagConfigMapping());
    }
  ...
  void removeTagConfigById(final Long tagId) {
    if (!indicesTransport.exists(this.configIndex)) {
      return;
    }
